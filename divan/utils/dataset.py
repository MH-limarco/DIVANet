...
class base_Dataset(Dataset):
    def __init__(self,
        ...
        self.num_workers = max(num_workers, 12)
    ...
    def _pre_loading(self):
        ...
        if self.num_workers <= 1:
            if self._asyncio:
                asyncio.run(self._pre_loading_async(desc))
            else:
                pbar = range(self.__len__()) if self.silence else tqdm(range(self.__len__()),
                                                                       desc=desc,
                                                                       ncols=self.ncols)
                self.img_label = [self._get_image_and_label(i) for i in pbar]
    ...
    async def _pre_loading_async(self, desc):
        ...
        tasks = [loop.run_in_executor(None, self._get_image_and_label, i) for i in range(self.__len__())]

        _loop = asyncio.as_completed(tasks) if self.silence else tqdm(asyncio.as_completed(tasks),
                                                                              total=self.__len__(),
                                                                              desc=desc,
                                                                              ncols=self.ncols)
        for i, result in enumerate(_loop):
            self.img_label[i] = await result
...
class DIVANetDataset:
    def __init__(self,
        ...
        self.dataset = base_Dataset(dataset_path,
                                    label_path,
                                    transform,
                                    channels,
                                    channels_mode,
    ...
    def build_loader(self):
        ...
        self.DataLoader = DataLoader(self.dataset,
                                     ...
                                     num_workers=min(self.num_workers, 4 if not self.num_workers else 0),
...
if __name__ == '__main__':
    ...
    DataManager = DIVANetDataset('../../dataset/dataset', 'train.txt', RAM=False, pin_memory=True, silence=False)
    ...
    DataManager2 = DIVANetDataset('../../dataset/dataset', 'train.txt', RAM='auto', pin_memory=True, silence=False)
